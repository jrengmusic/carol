...
- **ZERO bail-out guards** — bail-outs are bugs. In C++, preconditions use an assert — never `if (not x) return;`. Result returns are correct and preferred.
...
- No bail-out guards. Preconditions use an assert — never `if (not x) return;`. Positive checks only. Result returns are correct and preferred.
